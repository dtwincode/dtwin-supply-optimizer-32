...
-- Create the missing inventory_ddmrp_buffers_view with proper DDMRP fields
...
SELECT 
  ...
  -- DDMRP Buffer Zones (no safety stock)
  COALESCE(
  ...
  -- Buffer Zones Calculation (Red Zone = ADU * DLT * LT Factor * Variability Factor)
  COALESCE(
    (SELECT adu_adj FROM adu_90d_view WHERE product_id = pl.product_id AND location_id = pl.location_id LIMIT 1) * 
