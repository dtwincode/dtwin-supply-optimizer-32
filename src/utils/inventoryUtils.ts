...
export const calculateBufferZones = async (item: InventoryItem): Promise<BufferZones> => {
  ...
  const averageDailyUsage = item.currentStock ? (item.currentStock / 30) * 0.8 : 1; // Estimate
 ...
 */
export const shouldCreatePurchaseOrder = (netFlowPosition: number, bufferZones: BufferZones): boolean => {
