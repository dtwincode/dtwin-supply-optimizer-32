...
export const RouteOptimizationForm = ({
  onRouteGenerated,
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="origin">
              {getTranslation("logistics.origin", language)}
            </Label>
            <Select value={origin} onValueChange={setOrigin}>
              <SelectTrigger>
                <SelectValue
                  placeholder={getTranslation(
                    "logistics.selectOriginLocation",
                    language
                  )}
                />
              </SelectTrigger>
          ...
          <div className="space-y-2">
            <Label htmlFor="destination">
              {getTranslation("logistics.destination", language)}
            </Label>
            <Select value={destination} onValueChange={setDestination}>
              <SelectTrigger>
                <SelectValue
                  placeholder={getTranslation(
                    "logistics.selectDestinationLocation",
                    language
                  )}
                />
              </SelectTrigger>
          ...
          <div className="space-y-2">
            <Label htmlFor="optimizationCriteria">
              {getTranslation("logistics.optimizationCriteria", language)}
            </Label>
            <Select
              value={optimizationCriteria}
              onValueChange={(value) =>
                setOptimizationCriteria(value as "time" | "cost" | "emissions")
              }
            >
              <SelectTrigger>
                <SelectValue
                  placeholder={getTranslation(
                    "logistics.selectOptimizationCriteria",
                    language
                  )}
                />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="time">
                  {getTranslation("logistics.fastestTime", language)}
                </SelectItem>
                <SelectItem value="cost">
                  {getTranslation("logistics.lowestCost", language)}
                </SelectItem>
                <SelectItem value="emissions">
                  {getTranslation("logistics.lowestEmissions", language)}
                </SelectItem>
              </SelectContent>
            </Select>
          </div>
          ...
          >
            {loading ? (
              <>
                ...
                {getTranslation("logistics.calculatingOptimalRoute", language)}
              </>
            ) : (
              getTranslation("logistics.calculateOptimalRoute", language)
            )}
          </Button>
        </form>
      </CardContent>
