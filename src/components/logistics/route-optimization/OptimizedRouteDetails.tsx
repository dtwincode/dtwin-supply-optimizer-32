...
export const OptimizedRouteDetails = ({
  route,
  onReset,
}: OptimizedRouteDetailsProps) => {
  ...
  const handleSaveRoute = async () => {
    ...
    try {
      ...
      toast.success(getTranslation("logistics.routeSaved", language));
    } catch (error) {
      console.error("Error saving route:", error);
      toast.error(getTranslation("logistics.failedToSaveRoute", language));
    } finally {
      setSaving(false);
    }
  };
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        >
          {route.status === "planned"
            ...
            : route.status === "in-progress"
              ? getTranslation("logistics.status.inProgress", language)
              : getTranslation("logistics.status.completed", language)}
        </Badge>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <h3 className="text-sm font-medium text-muted-foreground">
                {getTranslation("logistics.routeDetails", language)}
              </h3>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {getTranslation("logistics.transitTime", language)}
                  </p>
                </div>
              </div>
            ...
            <div className="flex items-center space-x-4">
              <div className="flex items-center space-x-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {getTranslation("logistics.co2Emissions", language)}
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center space-x-2">
                ...
                <div>
                  <p className="text-sm font-medium">
                    {route.fuel_consumption_liters} L
                  </p>
                  <p className="text-xs text-muted-foreground">
                    {getTranslation("logistics.fuelConsumption", language)}
                  </p>
                </div>
              </div>
            </div>
              ...
              >
                {saving ? (
                  <>
                    ...
                    {getTranslation("logistics.saving", language)}
                  </>
                ) : (
                  <>
                    <Save className="mr-2 h-4 w-4" />
                    {getTranslation("logistics.saveRoute", language)}
                  </>
                )}
              </Button>
              <Button variant="outline" onClick={onReset} className="flex-1">
                <RotateCcw className="mr-2 h-4 w-4" />
                {getTranslation("logistics.reset", language)}
              </Button>
            </div>
          </div>
        </div>
