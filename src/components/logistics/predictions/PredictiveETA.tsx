...
import { Clock, TrendingUp, TrendingDown, AlertTriangle, Eye } from 'lucide-react';
  ...
  const [predictions, setPredictions] = useState<ShipmentPrediction[]>([
    {
      ...
      confidence: 92,
      factors: ['Weather in Jeddah', 'Traffic congestion', 'Historical carrier performance'],
      status: 'delayed',
    ...
    },
    {
      ...
      confidence: 76,
      factors: ['Long-haul route variability', 'Weather forecast changes', 'Historical delays'],
      status: 'at_risk',
          ...
          <TableBody>
            {predictions.map((prediction) => (
              <TableRow key={prediction.id} className="hover:bg-muted/30">
                <TableCell className="font-medium">{prediction.reference}</TableCell>
                <TableCell>
                  ...
                  <div className="flex items-center">
                    ...
                    {prediction.status !== 'on_time' && (
                      <span className="ml-2 text-sm font-medium text-muted-foreground">
                        {getTimeDifference(prediction.originalETA, prediction.predictedETA)}
                      </span>
                    )}
