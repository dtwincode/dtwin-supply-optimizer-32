...
import { Cloud, CloudRain, Sun, Wind, CloudLightning, AlertTriangle } from 'lucide-react';
import { useLanguage } from '@/contexts/LanguageContext';
import { getTranslation } from '@/translations';
import { fetchWeatherForecast, WeatherData, calculateWeatherImpact } from '@/utils/forecasting/weather';
...
export const WeatherImpactAnalysis: React.FC = () => {
        ...
        const updatedLocations = await Promise.all(
          locations.map(async (loc) => {
            try {
              ...
              const impact = weather.weatherCondition.includes('rain') || 
                             weather.weatherCondition.includes('storm') ? 
                             'medium' : weather.temperature > 40 ? 
                             'high' : 'low';
  ...
  const calculateRisk = (weather: WeatherData) => {
    ...
    if (weather.weatherCondition.includes('storm') || weather.weatherCondition.includes('thunder')) risk += 40;
  ...
  const getWeatherIcon = (weather: WeatherData | null) => {
    ...
    } else if (weather.weatherCondition.includes('clear') || weather.weatherCondition.includes('sun')) {
      return <Sun className="h-6 w-6 text-amber-500" />;
    } else if (weather.weatherCondition.includes('storm') || weather.weatherCondition.includes('thunder')) {
    ...
    <Card className="shadow-md">
      <CardHeader>
        <CardTitle className="text-xl">
          {getTranslation('common.logistics.weatherImpact', language) || "Weather Impact Analysis"}
        </CardTitle>
      </CardHeader>
      <CardContent>
        {locations.some(loc => loc.delayRisk > 20) && (
          <Alert className="mb-4 bg-amber-50 border-amber-200">
            ...
            <AlertDescription className="text-amber-800">
              {getTranslation('common.logistics.weatherAlert', language) || 
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {locations.map((location) => (
            <div key={location.name} className="flex items-start space-x-4 p-4 border rounded-lg">
              ...
              <div className="flex-1">
                <div className="flex justify-between items-center">
                  ...
                  <Badge className={getImpactColor(location.impact)}>
                    {location.impact.charAt(0).toUpperCase() + location.impact.slice(1)} Impact
                  </Badge>
                </div>
                  ...
                  <div className="w-full bg-gray-200 rounded-full h-2.5 mt-1">
                    <div 
                      className={`h-2.5 rounded-full ${
                        location.delayRisk > 50 ? 'bg-red-600' : 
                        location.delayRisk > 25 ? 'bg-amber-500' : 'bg-green-500'
                      }`} 
