...
const outputFormats = [
  { id: "text", name: { en: "Text Response", ar: "استجابة نصية" } },
  { id: "chart", name: { en: "Chart Visualization", ar: "تصور بياني" } },
  { id: "report", name: { en: "Detailed Report", ar: "تقرير مفصل" } },
];

// Context about the application to provide to the AI
const appContext = `... same as before ...`;
  ...
  const handleSubmit = async () => {
    if (!query.trim()) {
      toast({
        ...
        description:
          language === "ar" ? "الرجاء إدخال استفسار" : "Please enter a query",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      const functionUrl =
        "https://mttzjxktvbsixjaqiuxq.supabase.co/functions/v1/process-ai-query";
      ...
      const response = await fetch(functionUrl, {
        ...
        headers: {
          ...
          "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im10dHpqeGt0dmJzaXhqYXFpdXhxIiwicm9sZSI6ImFub24iLCJpYXQiOjE3MzkxNjk4NDEsImV4cCI6MjA1NDc0NTg0MX0.-6wiezDQfeFz3ecyuHP4A6QkcRRxBG4j8pxyAp7hkx8",
      ...
      setError(
        error.message || "Failed to process your query. Please try again."
      );
      toast({
        title: "Error",
        description:
            ...
            {messages.length === 0 && !error && (
              <div className="flex items-center justify-center h-64 text-muted-foreground">
                <div className="text-center">
                  ...
                  <p className="text-lg">
                    {language === "ar"
                      ? "اطرح أي سؤال حول بيانات سلسلة التوريد الخاصة بك"
                      : "Ask anything about your supply chain data"}
                  </p>
                  <p className="text-sm mt-2">
                    {language === "ar"
                      ? 'أمثلة: "ما المنتجات التي لديها أعلى تذبذب في الطلب؟", "حلل اتجاهات دقة التوقعات", "ما المنتجات ذات أطول أوقات التوريد؟"'
                      : 'Examples: "What products have the highest demand variability?", "Analyze my forecast accuracy trends", "Which products have the longest lead times?"'}
                  </p>
                </div>
              </div>
            )}
                ...
                <SelectContent>
                  {outputFormats.map((format) => (
                    <SelectItem key={format.id} value={format.id}>
                      {language === "ar" ? format.name.ar : format.name.en}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
